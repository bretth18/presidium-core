...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("presidium"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("presidium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "presidium:");
    }
...
{
    QString ret = QString("presidium:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "presidium.desktop";
    return GetAutostartDir() / strprintf("presidium-%s.lnk", chain);
}
